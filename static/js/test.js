...
function renderLandingPage(data) {
    ...
    createCircularStat(statsContainer, 10000, "Movies & TV Shows");
    createCircularStat(statsContainer, 500, "Genres & Categories");
    createCircularStat(statsContainer, 12, "Download Sources");
    createCircularStat(statsContainer, 24, "Countries");
...
function createCircularStat(container, targetValue, label) {
    ...
    statProgress.setAttribute('r', '45');
    statProgress.setAttribute('fill', 'none');
    statProgress.setAttribute('stroke', '#f5c518');
    statProgress.setAttribute('stroke-width', '8');
    statProgress.setAttribute('stroke-dasharray', '0 283'); // 2 * π * r ≈ 283
    ...
    const observer = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                animateStat(statProgress, statNumber, targetValue);
...
function animateStat(progressElement, numberElement, targetValue) {
    ...
    async function renderFavorites() {
        ...
        if (allFavorites.length > 0) {
            ...
            allFavorites.forEach((fav, index) => {
                // Avoid duplicates
                if (index > 0 && allFavorites[index-1].title === fav.title) {
                    return;
                }
